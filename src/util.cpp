...
int nAnonymizeBananaBitsAmount = 500;
...
#else
    const char* pszModule = "bananabits";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BananaBits
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BananaBits
    // Mac: ~/Library/Application Support/BananaBits
    // Unix: ~/.bananabits
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BananaBits";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bananabits.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bananabitsd.pid"));
