...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBananaBitsAmount"))
        settings.setValue("nAnonymizeBananaBitsAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeBananaBitsAmount = settings.value("nAnonymizeBananaBitsAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeBananaBitsAmount"))
        SoftSetArg("-anonymizebananabitsamount", settings.value("nAnonymizeBananaBitsAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeBananaBitsAmount:
            nAnonymizeBananaBitsAmount = value.toInt();
            settings.setValue("nAnonymizeBananaBitsAmount", nAnonymizeBananaBitsAmount);
            emit anonymizeBananaBitsAmountChanged(nAnonymizeBananaBitsAmount);
            break;
        default:
            break;
        }
