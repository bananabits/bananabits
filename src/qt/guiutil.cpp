...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bananabits"))
        return false;
...
{
    // Convert bananabits:// to bananabits:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bananabits://"))
    {
        uri.replace(0, 12, "bananabits:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BananaBits.lnk";
}
...
{
    return GetAutostartDir() / "bananabits.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BananaBits-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bananabits-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BananaBits-Qt"));
